...
func handler(
	...
	analytics := []helix.GameAnalytic{}
	var bookmark string

	// Get all possible game analytics retrievable
	// by the user
	for {
		responses, cursor, err := twitchInternal.GetGameAnalytics(
			"",
			nil,
			nil,
			10,
		...
		log.Printf("found %d game analytics", len(responses.GameAnalytics))

		analytics = append(analytics, responses.GameAnalytics...)
	...
	for _, analytic := range analytics {
		...
		log.Printf(
			"got new game analytic file type:%q url:%q end-date:%q",
			analytic.Type,
			analytic.URL,
